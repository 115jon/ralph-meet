...
if (!ACCOUNT_ID || !API_TOKEN) {
  console.error(
    "❌ Missing CLOUDFLARE_LOGS_ACCOUNT_ID or CLOUDFLARE_LOGS_API_TOKEN.",
  );
  console.error("");
  ...
  console.error("");
  console.error(
    "You can generate an API Token at https://dash.cloudflare.com/profile/api-tokens",
  );
...
console.log(
  `Config: limit=${limit}, search=${search || "<none>"}, timeframe=past ${hours} hour(s), json=${isJson}`,
);
...
try {
  ...
  if (Array.isArray(results) && results.length > 0) {
    ...
    for (const event of results) {
      ...
      const scriptName =
        event.$workers?.scriptName || event.ScriptName || "unknown";
      const entrypoint = event.$workers?.entrypoint
        ? ` [${event.$workers.entrypoint}]`
        : "";
      const level = event.source?.level
        ? `[${event.source.level.toUpperCase()}] `
        : "";
      ...
      if (Array.isArray(event.logs) && event.logs.length > 0) {
        for (const l of event.logs) {
          console.log(
            `   [Worker Log ${l.level || "INFO"}]: ${l.message?.join(" ") || l.message || JSON.stringify(l)}`,
          );
        }
      }
      if (Array.isArray(event.Exceptions) && event.Exceptions.length > 0) {
        for (const e of event.Exceptions) {
          console.log(
            `   [Worker EXCEPTION]: ${e.message || JSON.stringify(e)}`,
          );
        }
      }
      if (Array.isArray(event.exceptions) && event.exceptions.length > 0) {
