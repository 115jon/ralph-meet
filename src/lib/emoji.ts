...
const NATIVE_CATEGORY_META: Record<
  string,
  { label: string; iconShortcodes: string[] }
> = {
  people: {
...
function buildAliasMap() {
  ...
  for (const list of aliasesById.values()) {
    list.sort(
      (left, right) =>
        scoreAlias(left) - scoreAlias(right) ||
...
function normalizeNativeEmojiSkinTone(
...
function applyNativeEmojiSkinToneInternal(
  emoji: NativeEmoji,
  tone: NativeEmojiSkinTone,
): NativeEmoji {
  const skin =
    emoji.skins[Math.min(tone, emoji.skins.length - 1)] ?? emoji.skins[0];
...
for (const category of martData.categories ?? []) {
  ...
  for (const emojiId of category.emojis ?? []) {
    ...
    const shortcodes = Array.from(
      new Set([...sanitizedAliases, sanitizedId].filter(Boolean)),
    );
    ...
    const emoji: NativeEmoji = {
      ...
      keywords: Array.from(
        new Set(
          [...(source.keywords ?? []), ...(source.emoticons ?? [])]
    ...
    for (const skin of skins) {
      ...
      for (const nativeValue of new Set([
        skin.native,
        normalizeNativeEmojiValueKey(skin.native),
      ])) {
      ...
      meta?.iconShortcodes
        .map((shortcode) => {
          ...
          return (
            categoryItems.find(
              (emoji) =>
                emoji.shortcodes.includes(normalized) ||
                emoji.preferredShortcode === normalized,
            ) ?? null
          );
        })
...
const nativeEmojiPattern = Array.from(new Set(nativeEmojiByValue.keys()))
  ...
  .map(escapeRegex)
  .join("|");
const nativeEmojiRegex = nativeEmojiPattern
  ? new RegExp(nativeEmojiPattern, "g")
  : null;

export function applyNativeEmojiSkinTone(
  emoji: NativeEmoji,
  tone: number | null | undefined = 0,
): NativeEmoji {
  return applyNativeEmojiSkinToneInternal(
    emoji,
    normalizeNativeEmojiSkinTone(tone),
  );
}

export function getNativeEmojiCategories(
  tone: number | null | undefined = 0,
): NativeEmojiCategory[] {
  return nativeEmojiCategories.map((category) => ({
    ...category,
    emojis: category.emojis.map((emoji) =>
      applyNativeEmojiSkinTone(emoji, tone),
    ),
  }));
}

export function getNativeEmojiById(
  id: string,
  tone: number | null | undefined = 0,
): NativeEmoji | null {
...
export function resolveNativeEmojiValue(value: string): NativeEmoji | null {
  return (
    nativeEmojiByValue.get(value) ??
    nativeEmojiByValue.get(normalizeNativeEmojiValueKey(value)) ??
    null
  );
}

export function resolveNativeEmojiShortcode(
  shortcode: string,
  tone: number | null | undefined = 0,
): NativeEmoji | null {
  const normalized = sanitizeShortcodeCandidate(shortcode);
...
export function splitTextByNativeEmoji(text: string): NativeEmojiTextSegment[] {
  ...
  while ((match = nativeEmojiRegex.exec(text)) !== null) {
    ...
    const emoji =
      nativeEmojiByValue.get(match[0]) ??
      nativeEmojiByValue.get(normalizeNativeEmojiValueKey(match[0]));
    if (emoji) {
...
export function searchNativeEmojis(
  query: string,
  limit = 120,
  tone: number | null | undefined = 0,
): NativeEmoji[] {
    ...
    .map((emoji) => {
      ...
      for (const term of terms) {
        if (emoji.preferredShortcode === term) score += 120;
          ...
          emoji.shortcodes.some((shortcode) => shortcode.startsWith(term))
        )
          score += 60;
  ...
  return scored
    .slice(0, limit)
    .map((entry) => applyNativeEmojiSkinTone(entry.emoji, tone));
}

export function sanitizeGeneratedEmojiShortcode(
  value: string,
  maxLength = 32,
): string {
  ...
  return (
    (normalized || "emoji").slice(0, maxLength).replace(/^_+|_+$/g, "") ||
    "emoji"
  );
}
...
export function buildGeneratedEmojiStorageKey(
  userId: string,
  id: string,
  contentType: string,
): string {
...
export function parseCustomEmojiToken(
  value: string,
): { shortcode: string; id: string } | null {
...
export function toCustomEmojiRecentItem(
  emoji: Pick<
    GeneratedEmoji,
    "id" | "shortcode" | "prompt" | "image_url" | "token"
  >,
): EmojiRecentItem {
  return {
    type: "custom",
