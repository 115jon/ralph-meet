...
const VALID_MEDIA_CONTENT_TYPES = new Set<
...
export function sanitizeVoiceChannelStatusMedia(
  value: unknown,
): VoiceChannelStatusMedia | null {
  if (!value || typeof value !== "object") return null;

  const candidate = value as Record<string, unknown>;
  const previewUrl =
    typeof candidate.preview_url === "string"
      ? candidate.preview_url.trim()
      : "";
  const previewWidth =
    typeof candidate.preview_width === "number"
      ? candidate.preview_width
      : Number(candidate.preview_width);
  const previewHeight =
    typeof candidate.preview_height === "number"
      ? candidate.preview_height
      : Number(candidate.preview_height);
  ...
  if (
    ...
    !VALID_MEDIA_CONTENT_TYPES.has(
      previewContentType as VoiceChannelStatusMedia["preview_content_type"],
    )
  ) {
    return null;
  }
  ...
  return {
    ...
    provider:
      typeof candidate.provider === "string" && candidate.provider.trim()
        ? candidate.provider
        : "external",
    media_type: isGifPickerMediaType(candidate.media_type)
    ...
    title:
      typeof candidate.title === "string" && candidate.title.trim()
        ...
        : null,
    alt_text:
      typeof candidate.alt_text === "string" && candidate.alt_text.trim()
        ? candidate.alt_text.trim()
        : null,
    source_url:
...
export function sanitizeVoiceChannelStatus(
  value: unknown,
...
export function hasVoiceChannelStatus(
  status: VoiceChannelStatus | null | undefined,
): boolean {
  return Boolean(status?.text || status?.media);
}

export function isVoiceChannelStatusVideo(
  media: VoiceChannelStatusMedia | null | undefined,
): boolean {
...
export function voiceChannelStatusMediaFromGifItem(
  item: GifPickerItem,
