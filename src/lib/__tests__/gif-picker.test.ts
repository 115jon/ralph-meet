...
describe("gif-picker helpers", () => {
  function makeGif(id: string) {
    return normalizeTenorGifResult({
      ...
      media_formats: {
        gif: {
          url: `https://media1.tenor.com/m/${id}.gif`,
          dims: [200, 200],
          size: 1234,
        },
        tinymp4: {
          url: `https://media.tenor.com/${id}.mp4`,
          dims: [100, 100],
          size: 200,
        },
      },
  ...
  function makeKlipyGif(id: string) {
    return normalizeKlipyGifResult({
      ...
      media_formats: {
        ...
        tinymp4: {
          url: `https://static.klipy.com/${id}.mp4`,
          dims: [160, 120],
          size: 200,
        },
      },
  ...
  it("normalizes a Tenor result into preview and send assets", () => {
    const item = normalizeTenorGifResult({
      ...
      media_formats: {
        ...
        tinygif: {
          url: "https://media.tenor.com/goat-small.gif",
          dims: [165, 294],
          size: 720402,
        },
        mp4: {
          url: "https://media.tenor.com/goat.mp4",
          dims: [282, 498],
          size: 206228,
        },
        tinymp4: {
  ...
  it("normalizes a KLIPY result into preview and send assets", () => {
    const item = normalizeKlipyGifResult({
      ...
      media_formats: {
        ...
        tinygif: {
          url: "https://static.klipy.com/tiny.gif",
          dims: [220, 220],
          size: 149153,
        },
        mp4: {
          url: "https://static.klipy.com/full.mp4",
          dims: [498, 498],
          size: 91000,
        },
        tinymp4: {
  ...
  it("toggles favorites by id and preserves newest first ordering", () => {
    const first = normalizeTenorGifResult({
      ...
      media_formats: {
        gif: {
          url: "https://media1.tenor.com/m/1.gif",
          dims: [200, 200],
          size: 1234,
        },
        tinymp4: {
          url: "https://media.tenor.com/1.mp4",
          dims: [100, 100],
          size: 200,
        },
      },
    ...
    const second = normalizeTenorGifResult({
      ...
      media_formats: {
        gif: {
          url: "https://media1.tenor.com/m/2.gif",
          dims: [200, 200],
          size: 1234,
        },
        tinymp4: {
          url: "https://media.tenor.com/2.mp4",
          dims: [100, 100],
          size: 200,
        },
      },
  ...
  it("parses stored favorites safely", () => {
    ...
    expect(
      parseStoredGifFavorites(
        JSON.stringify([
          { id: "1", preview: { url: "a" }, send: { url: "b" } },
        ]),
      ),
    ).toHaveLength(1);

    const parsed = parseStoredGifFavorites(
      JSON.stringify([
        {
          id: "gif-1",
          preview: {
            url: "https://static.klipy.com/gif-1.gif",
            contentType: "image/gif",
          },
          send: {
            url: "https://static.klipy.com/gif-1.gif",
        ...
        },
        {
          id: "sticker-1",
          preview: {
            url: "https://static.klipy.com/stickers/sticker-1.png",
        ...
        },
        {
          id: "clip-1",
          preview: {
            url: "https://static.klipy.com/clips/clip-1.mp4",
            contentType: "video/mp4",
          },
          send: {
            url: "https://static.klipy.com/clips/clip-1.mp4",
        ...
        },
        {
          id: "meme-1",
          preview: {
            url: "https://static.klipy.com/static-memes/meme-1.png",
            contentType: "image/png",
          },
          send: {
            url: "https://static.klipy.com/static-memes/meme-1.png",
  ...
  it("normalizes static image content types and infers meme URLs without hijacking generic pngs", () => {
    expect(normalizeGifPickerContentType("image/png; charset=binary")).toBe(
      "image/png",
    );
    ...
    expect(
      inferGifPickerMediaType({
        ...
        preview: {
          url: "https://static.klipy.com/static-memes/meme-asset.png",
          contentType: "image/png",
        },
        send: {
          url: "https://static.klipy.com/static-memes/meme-asset.png",
    ...
    expect(
      inferGifPickerMediaType({
        ...
        preview: {
          url: "https://cdn.example.com/plain-image.png",
          contentType: "image/png",
        },
        send: {
          url: "https://cdn.example.com/plain-image.png",
  ...
  it("builds stable Tenor cache keys for equivalent params", () => {
    expect(
      buildTenorCacheKey("/search", { q: "goat", limit: 24, pos: "abc" }),
    ).toBe(buildTenorCacheKey("/search", { pos: "abc", limit: 24, q: "goat" }));
  });

  it("dedupes GIF results by id while preserving first-seen order", () => {
    expect(
      dedupeGifPickerItems([makeGif("1"), makeGif("2"), makeGif("1")]).map(
        (item) => item.id,
      ),
    ).toEqual(["1", "2"]);
    expect(
  ...
  it("treats same ids from different providers as distinct GIFs", () => {
    expect(
      dedupeGifPickerItems([makeGif("1"), makeKlipyGif("1")]).map(
        (item) => item.provider,
      ),
    ).toEqual(["tenor", "klipy"]);
  });
  ...
  it("detects GIF providers from attachment paths", () => {
    expect(
      getGifAttachmentProvider(
        "attachments/channel/attachment/gifs/klipy/test.gif",
      ),
    ).toBe("klipy");
    expect(
      getGifAttachmentProvider(
        "/api/attachments/channel/attachment/gifs/tenor/test.gif",
      ),
    ).toBe("tenor");
    expect(getGifAttachmentProvider("https://static.klipy.com/test.gif")).toBe(
      "klipy",
    );
    expect(getGifAttachmentProvider("https://static1.klipy.com/test.mp4")).toBe(
      "klipy",
    );
    expect(getGifAttachmentProvider("https://media.tenor.com/test.gif")).toBe(
      "tenor",
    );
    expect(
      getGifAttachmentProvider(
    ...
    expect(
      getGifAttachmentProvider(
        "/api/proxy-media?url=https%3A%2F%2Fmedia.tenor.com%2Ftest.gif",
      ),
    ).toBe("tenor");
    expect(
      getGifAttachmentProvider(
        "/api/proxy-media?url=https%3A%2F%2Fgif.fxtwitter.com%2Ftweet_video%2Ftest.webp",
      ),
    ).toBe("external");
    expect(
      getGifAttachmentProvider("attachments/channel/attachment/test.gif"),
    ).toBeNull();
    expect(
      getGifAttachmentProvider("https://cdn.example.com/test.gif"),
    ).toBeNull();
  });
});
