...
describe("platform asset urls", () => {
  it("proxies Google user-content avatar urls through proxy-media", () => {
    expect(getAuthAssetUrl("https://lh3.googleusercontent.com/a/example-avatar=s96-c")).toBe(
  ...
  it("leaves other external asset urls unchanged", () => {
    expect(getAuthAssetUrl("https://example.com/image.png")).toBe("https://example.com/image.png");
  });
});
