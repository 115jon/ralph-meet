...
function browserAudioDeviceCount(
...
function nativeAudioDeviceCount(
  devices: NativeDevice[],
  kind: "audioinput" | "audiooutput",
) {
  return devices.filter((device) => device.kind === kind && !device.is_default)
    .length;
}
...
function hasCompleteDesktopDeviceList(
  ...
  const nativeInputCount = nativeAudioDeviceCount(
    nativeAudioDevices,
    "audioinput",
  );
  const nativeOutputCount = nativeAudioDeviceCount(
    nativeAudioDevices,
    "audiooutput",
  );
  const browserInputCount = browserAudioDeviceCount(
    browserDevices,
    "audioinput",
  );
  const browserOutputCount = browserAudioDeviceCount(
    browserDevices,
    "audiooutput",
  );
  ...
  const nativeVideoCount = nativeVideoDevices.filter(
    (d) => d.kind === "videoinput",
  ).length;
  const browserVideoCount = browserDevices.filter(
    (d) => d.kind === "videoinput" && d.label.trim().length > 0,
  ).length;

  const videoComplete =
    nativeVideoCount === 0 || browserVideoCount >= nativeVideoCount;
...
export async function enumerateMediaDevicesWithRetry() {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    mediaLog.debug("Desktop device list not ready yet; retrying enumeration", {
      ...
      browserVideoInputs: lastDevices.filter(
        (d) => d.kind === "videoinput" && d.label.trim().length > 0,
      ).length,
      nativeAudioInputs: nativeAudioDeviceCount(
        nativeAudioDevices,
        "audioinput",
      ),
      nativeAudioOutputs: nativeAudioDeviceCount(
        nativeAudioDevices,
        "audiooutput",
      ),
      nativeVideoInputs: nativeVideoDevices.filter(
        (d) => d.kind === "videoinput",
      ).length,
      devices: lastDevices.map((d) => ({
        kind: d.kind,
        label: d.label || "(empty)",
      })),
    });
...
function hasBrowserGeneratedLabel(
  device: MediaDeviceInfo_Custom,
  index: number,
) {
  const fallbackName =
...
export function mergeNativeAudioLabels(
  ...
  const defaultNativeDevice = matchingNativeDevices.find(
    (device) => device.is_default,
  );
  const nonDefaultNativeDevices = matchingNativeDevices.filter(
    (device) => !device.is_default,
  );
  ...
  const mergedBrowserDevices = browserDevices.map((device, index) => {
    ...
    if (!hasBrowserGeneratedLabel(device, index)) {
      const nativeMatch = matchingNativeDevices.find(
        (nativeDevice) =>
          normalizedDeviceLabel(nativeDevice.label) ===
          normalizedDeviceLabel(device.label),
      );
      if (nativeMatch) {
    ...
    const nativeDevice =
      nonDefaultNativeDevices[nonDefaultIndex] ??
      matchingNativeDevices[nonDefaultIndex];
    nonDefaultIndex++;
  ...
  if (hiddenNativeCount > 0) {
    mediaLog.debug(
      "Native devices not exposed by CEF/WebRTC were hidden from selectable list",
      {
        kind,
        hiddenNativeCount,
...
export function mergeNativeVideoLabels(
  ...
  const matchingNativeDevices = nativeDevices.filter(
  ...
  if (hiddenNativeCount > 0) {
    mediaLog.debug(
      "Native video devices not exposed by CEF/WebRTC were hidden from selectable list",
      {
        hiddenNativeCount,
        browserDeviceCount: browserDevices.length,
        nativeDeviceCount: matchingNativeDevices.length,
...
export async function getMediaDeviceSnapshot(): Promise<MediaDeviceSnapshot> {
  ...
  const audioOutputs = rawDevices
    ...
    .map((d, i) => ({
      ...
      label:
        d.label ||
        (d.deviceId === "default" ? "Default Speaker" : `Speaker ${i + 1}`),
      kind: d.kind,
    }));
  ...
  return {
    ...
    nativeAudioDevices,
    nativeVideoDevices,
    audioInputs: mergeNativeAudioLabels(
      audioInputs,
      nativeAudioDevices,
      "audioinput",
    ),
    audioOutputs: mergeNativeAudioLabels(
      audioOutputs,
      nativeAudioDevices,
      "audiooutput",
    ),
