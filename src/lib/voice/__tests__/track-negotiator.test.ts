...
describe("TrackNegotiator", () => {
  ...
  describe("publishTracks", () => {
    it("should add transceivers and create an offer for audio and video", async () => {
      ...
      const selectProtocolCall = (mockConfig.sendWS as any).mock.calls.find(
        (c: any) => c[0].op === 1,
      );
      expect(selectProtocolCall).toBeDefined();
      expect(selectProtocolCall[0].d.push_tracks).toHaveLength(2);

      const tracksReadyCall = (mockConfig.sendWS as any).mock.calls.find(
        (c: any) => c[0].op === 102,
      );
    ...
    it("should configure single stream for screen share video", async () => {
      ...
      expect(pushPC.addTransceiver).toHaveBeenCalledWith(
        publishedTrack,
        expect.objectContaining({
          direction: "sendonly",
          sendEncodings: [
    ...
    it("retries addTransceiver without unsupported RTP parameters", async () => {
      ...
      const originalAddTransceiver =
        pushPC.addTransceiver.getMockImplementation();
  ...
  describe("handleSessionDescription", () => {
    ...
    it("should process pull offer and create answer", async () => {
      ...
      const answerCall = (mockConfig.sendWS as any).mock.calls.find(
        (c: any) => c[0].op === 14,
      );
    ...
    it("should leave pulled track metadata unchanged when pull offer SDP fails", async () => {
      ...
      pullPC.setRemoteDescription.mockRejectedValueOnce(
        new DOMException(
          "The order of m-lines in subsequent offer does not match",
          "InvalidAccessError",
        ),
      );
      ...
      await expect(
        negotiator.handleSessionDescription(payload, "pull"),
      ).rejects.toThrow("m-lines");

      expect(negotiator.pulledTracks).toEqual([existingTrack]);
      expect(mockConfig.sendWS).not.toHaveBeenCalledWith(
        expect.objectContaining({ op: 14 }),
      );
    });
  });
