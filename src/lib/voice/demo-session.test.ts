...
describe("demo sessions", () => {
  it("creates a signed HttpOnly session and resumes it from a cookie", async () => {
    const first = await getOrCreateDemoSession(new Request("https://meet.test/api/voice/socket-ticket"), secret, 1_000);
    ...
    const second = await getOrCreateDemoSession(new Request("https://meet.test/api/voice/socket-ticket", {
      headers: { cookie },
    }), secret, 2_000);
  ...
  it("replaces a tampered or expired cookie", async () => {
    ...
    const replacement = await getOrCreateDemoSession(new Request("https://meet.test/api/voice/socket-ticket", {
      headers: { cookie: tampered },
    }), secret, 2_000);
    expect(replacement.cookie).not.toBeNull();
    expect(replacement.subject).not.toBe(first.subject);

    const expired = await getOrCreateDemoSession(new Request("https://meet.test/api/voice/socket-ticket", {
      headers: { cookie },
    }), secret, 1_000 + 24 * 60 * 60 * 1000);
