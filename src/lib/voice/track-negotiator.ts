...
function isUnsupportedRtpParameterError(error: unknown): boolean {
  ...
  return (
    ...
    /unimplemented parameter|RtpParameters/i.test(message)
  );
}

function compatibleEncoding(
  ...
  if (typeof encoding.maxBitrate === "number")
    next.maxBitrate = encoding.maxBitrate;
  if (typeof encoding.maxFramerate === "number")
    next.maxFramerate = encoding.maxFramerate;
  if (typeof encoding.scaleResolutionDownBy === "number")
    next.scaleResolutionDownBy = encoding.scaleResolutionDownBy;
  return next;
}
...
function addTransceiverWithEncodingFallback(
  ...
  try {
    ...
    if (!init.sendEncodings?.length || !isUnsupportedRtpParameterError(error))
      throw error;
    ...
    pushCam.warn(
      `Retrying ${trackName} without advanced RTP encoding parameters`,
      error,
    );
...
function logScreenCodecCapabilities(trackName: string) {
  ...
  try {
    ...
    const mediaCodecs = codecs.filter(
      (codec) => !/\/(rtx|red|ulpfec)$/i.test(codec.mimeType),
    );
    const codecNames = mediaCodecs.map((codec) =>
      codec.mimeType.replace(/^video\//i, "").toUpperCase(),
    );

    pushCam.info(
      `Available browser codecs: ${JSON.stringify(
    ...
    pushCam.info(
      `Video codecs: ${codecNames.map((codec) => `${codec}[encode: true, decode: true]`).join(",")}`,
    );
    pushCam.info(
      `Screen codec capabilities for ${trackName}; using browser codec order`,
      {
...
function shouldClonePublishedTrack(
  prefix: string,
  track: MediaStreamTrack,
): boolean {
...
export class TrackNegotiator {
  ...
  get camPushPC(): RTCPeerConnection | null {
    return this.camPush.pc;
  }
  set camPushPC(pc: RTCPeerConnection | null) {
    this.camPush.pc = pc;
  }

  /** Public accessor for the screen push PC */
  get screenPushPC(): RTCPeerConnection | null {
    return this.screenPush.pc;
  }
  set screenPushPC(pc: RTCPeerConnection | null) {
    this.screenPush.pc = pc;
  }

  /** Get cam session ID */
  get camPushSessionId(): string | null {
    return this.camPush.sessionId;
  }

  /** Get screen session ID */
  get screenPushSessionId(): string | null {
    return this.screenPush.sessionId;
  }

  /** All published track names across both PCs */
  get publishedTrackNames(): Set<string> {
    return new Set([
      ...this.camPush.publishedTrackNames,
      ...this.screenPush.publishedTrackNames,
    ]);
  }
  ...
  public async publishTracks(
    ...
    if (DEBUG)
      pushCam.info(
        `publishTracks called: tracks=${stream.getTracks().length}, kinds=${stream
          .getTracks()
          .map((t) => t.kind)
          .join(",")}, pc=${!!ctx.pc}`,
      );
      ...
      .then(async () => {
        ...
        for (const originalTrack of stream.getTracks()) {
          ...
          if (transceiver) {
            ...
            transceiver = addTransceiverWithEncodingFallback(
              pushPC,
              track,
              {
                direction: "sendonly",
            ...
            if (
              ...
              try {
                ...
                if (caps?.codecs) {
                  const opusCodecs = caps.codecs.filter(
            ...
            if (track.kind === "video") {
              ...
              (parameters as any).degradationPreference =
                prefix === "screen" && track.contentHint !== "motion"
                  ? "maintain-resolution"
                  : "balanced";
              transceiver.sender
                ...
                .then(() => {
                  if (prefix === "screen") {
                    pushCam.info(
                      `Screen sender parameters applied for ${trackName}`,
                ...
                .catch((err) => {
                  pushCam.warn(
                    `degradationPreference failed for ${trackName}:`,
                    err,
                  );
                });
            }
        ...
        const mungedSDP = offer.sdp
          ? mungeStereoOpus(offer.sdp, prefix)
          : undefined;
        ...
        const negotiationDonePromise = this.config.waitForPushNegotiationDone(
          prefix as "cam" | "screen",
          10000,
        );
        const answerPromise = this.config.waitForPushAnswer(
          prefix as "cam" | "screen",
          10000,
        );
  ...
  public async handleSessionDescription(
    sd: SessionDescriptionPayload,
    type: "push" | "pull",
    prefix?: "cam" | "screen",
  ): Promise<void> {
    if (DEBUG)
      pushCam.info(
        `[Common] SessionDescription: session_id=${sd.session_id}, sdp_type=${sd.sdp_type}, tracks=${sd.tracks.length}, type=${type}, prefix=${prefix}`,
      );
    ...
    if (type === "pull" && sd.sdp_type === "offer") {
      ...
      if (sd.tracks) {
        for (const remote of sd.tracks) {
          ...
          const local = nextPulledTracks.find(
            (t) => t.track_name === remote.track_name,
          );
      ...
      try {
        ...
        this.pullPC.getTransceivers().forEach((tr) => {
          const track = tr.mid
            ? nextPulledTracks.find((t) => t.mid === tr.mid)
            : null;
          const isUnsubscribed =
            (tr.mid && this.config.getUnsubscribedMids().has(tr.mid)) ||
        ...
        await this.pullPC.setLocalDescription({
          type: "answer",
          sdp: mungedSDP,
        });
        ...
        this.config.sendWS({
          ...
          d: {
            sdp: this.pullPC.localDescription!.sdp,
      ...
      if (sd.ice_restart) {
        const ctx = sd.session_id
          ? this.camPush.sessionId === sd.session_id
            ? this.camPush
            : this.screenPush.sessionId === sd.session_id
              ? this.screenPush
              : null
          : null;
        if (ctx?.pc) {
          try {
            const remoteSdp = sd.sdp ? mungeStereoOpus(sd.sdp) : sd.sdp;
            await ctx.pc.setRemoteDescription({
              type: "answer",
              sdp: remoteSdp,
            });
            pushCam.info(
              `ICE restart answer applied for session ${sd.session_id}`,
            );
          } catch (err) {
      ...
      const ctx = prefix
        ? this.getPushContext(prefix)
        : this.routePushBySessionId(sd.session_id);

      if (!ctx) {
        pushCam.error(
          `Cannot route push answer — no matching context for session ${sd.session_id}`,
        );
        return;
      }
  ...
  private routePushBySessionId(sessionId: string): PushContext | null {
    ...
    if (
      this.camPush.pc?.signalingState === "have-local-offer" &&
      !this.camPush.sessionId
    ) {
      return this.camPush;
    }
    if (
      this.screenPush.pc?.signalingState === "have-local-offer" &&
      !this.screenPush.sessionId
    ) {
      return this.screenPush;
    }

    // Fallback: whichever is in have-local-offer
    if (this.camPush.pc?.signalingState === "have-local-offer")
      return this.camPush;
    if (this.screenPush.pc?.signalingState === "have-local-offer")
      return this.screenPush;
   ...
   */
  public async iceRestart(
    sessionType: "push_cam" | "push_screen" | "pull",
  ): Promise<void> {
   ...
   */
  public async handleIceRestartAnswer(
    sd: SessionDescriptionPayload,
  ): Promise<void> {
    if (this.pullPC && this.pullSessionId === sd.session_id) {
      try {
        ...
        pullLog.info(
          `ICE restart answer applied for pull session ${sd.session_id}`,
        );
      } catch (err) {
  ...
  public getPushTransceiver(trackName: string): RTCRtpTransceiver | undefined {
    return (
      this.camPush.transceivers.get(trackName) ||
  ...
  public teardownTransceiver(trackName: string) {
    ...
    const ctx = trackName.startsWith("screen-")
      ? this.screenPush
      : this.camPush;
      ...
      if (transceiver) {
        ...
        if (typeof transceiver.stop === "function") {
          try {
            ...
            console.warn("transceiver stop error:", e);
          }
        } else {
