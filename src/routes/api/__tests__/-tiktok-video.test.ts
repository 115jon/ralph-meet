...
describe("tiktok video route helpers", () => {
  it("canonicalizes supported TikTok lookup URLs", () => {
    expect(
      canonicalizeTikTokLookupUrl(
        "https://www.tiktok.com/@dj.giggle/photo/7656952653510888717?_r=1",
      ),
    ).toBe("https://www.tiktok.com/@dj.giggle/photo/7656952653510888717");
    expect(
      canonicalizeTikTokLookupUrl(
        "https://www.tiktok.com/t/ZTSSxgwxt/?lang=en",
      ),
    ).toBe("https://www.tiktok.com/t/ZTSSxgwxt/");
  });

  it("detects TikTok short lookup URLs", () => {
    expect(isTikTokShortLookupUrl("https://www.tiktok.com/t/ZTSSxgwxt/")).toBe(
      true,
    );
    expect(isTikTokShortLookupUrl("https://vm.tiktok.com/ZMabc123/")).toBe(
      true,
    );
    expect(
      isTikTokShortLookupUrl(
  ...
  it("deduplicates lookup urls when the resolved url matches the input", () => {
    ...
    ).toEqual(["https://www.tiktok.com/@dj.giggle/photo/7656952653510888717"]);
  });

  it("rejects unsupported TikTok lookup URLs", () => {
    expect(
      canonicalizeTikTokLookupUrl(
        "https://example.com/@dj.giggle/photo/7656952653510888717",
      ),
    ).toBeNull();
