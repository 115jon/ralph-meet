...
describe("listen together stream proxy helpers", () => {
  ...
  it("normalizes open-ended media ranges into bounded upstream requests", () => {
    expect(normalizeListenTogetherUpstreamRange("bytes=0-")).toBe("bytes=0-10485759");
    expect(normalizeListenTogetherUpstreamRange("bytes=65536-")).toBe("bytes=65536-10551295");
  });

  it("caps large closed media ranges to upstream-safe chunks", () => {
    expect(normalizeListenTogetherUpstreamRange("bytes=65536-3860871")).toBe("bytes=65536-3860871");
    expect(normalizeListenTogetherUpstreamRange("bytes=0-2048", { includeBody: false })).toBe("bytes=0-1");
  });

  it("uses a tiny probe range for HEAD-style metadata requests", () => {
    expect(normalizeListenTogetherUpstreamRange(null, { includeBody: false })).toBe("bytes=0-1");
    expect(normalizeListenTogetherUpstreamRange("bytes=2048-", { includeBody: false })).toBe("bytes=2048-2049");
  });
});
