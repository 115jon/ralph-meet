...
describe("instagram video route helpers", () => {
  it("canonicalizes supported Instagram media URLs", () => {
    expect(
      canonicalizeInstagramUrl(
        "https://www.instagram.com/reel/DXU4PV2AGJU/?igsh=abc",
      ),
    ).toBe("https://www.instagram.com/reel/DXU4PV2AGJU/");
    expect(canonicalizeInstagramUrl("https://instagram.com/p/ABC123")).toBe(
      "https://www.instagram.com/p/ABC123/",
    );
    expect(
      canonicalizeInstagramUrl("https://www.instagram.com/tv/XYZ987/"),
    ).toBe("https://www.instagram.com/tv/XYZ987/");
  });

  it("extracts shortcode from supported paths", () => {
    expect(
      extractInstagramShortcode("https://www.instagram.com/reel/DXU4PV2AGJU/"),
    ).toBe("DXU4PV2AGJU");
    expect(
      extractInstagramShortcode("https://www.instagram.com/p/ABC123/"),
    ).toBe("ABC123");
    expect(
      extractInstagramShortcode("https://www.instagram.com/reels/SHORT1/"),
    ).toBe("SHORT1");
  });

  it("normalizes instagram media ids to numeric media pk", () => {
    expect(normalizeInstagramMediaPk("3878972523924185684_71645946242")).toBe(
      "3878972523924185684",
    );
    expect(normalizeInstagramMediaPk("3878972523924185684")).toBe(
      "3878972523924185684",
    );
  ...
  it("rejects invalid or unsupported URLs", () => {
    expect(
      canonicalizeInstagramUrl("https://example.com/video/123"),
    ).toBeNull();
    expect(
      extractInstagramShortcode("https://www.instagram.com/explore/"),
    ).toBeNull();
  });
  ...
  it("returns null fields when graphql payload is missing media", () => {
    expect(
      parseInstagramGraphqlPayload({ data: { xdt_shortcode_media: null } }),
    ).toEqual({
      videoUrl: null,
      thumbnailUrl: null,
      title: null,
