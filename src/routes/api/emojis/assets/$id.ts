...
const GET = async ({ request, params }: any) => {
  ...
  headers.set(
    "Content-Type",
    record.content_type || object.httpMetadata?.contentType || "image/png",
  );
  headers.set("Cache-Control", "private, max-age=31536000, immutable");
  headers.set("Content-Length", object.size.toString());
  headers.set(
    "Content-Disposition",
    `inline; filename="${record.shortcode}.png"`,
  );
  headers.set("X-Content-Type-Options", "nosniff");
  headers.set(
    "Content-Security-Policy",
    "default-src 'none'; img-src 'self' data:; media-src 'self'; script-src 'none';",
  );
