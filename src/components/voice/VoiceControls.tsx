...
export function VoiceControls({
  ...
  const pillBgClass =
    "bg-rm-bg-surface/80 border-rm-border backdrop-blur-md shadow-2xl";
  const btnBaseClass =
    "text-rm-text bg-transparent hover:bg-rm-bg-hover hover:text-rm-text transition-colors";
  const btnFilledClass =
    "bg-rm-bg-hover text-rm-text hover:bg-rm-bg-active transition-colors";
    ...
    <>
      <div
        className={cn(
          ...
          variant === "default"
            ? "h-[72px] justify-between px-2 md:px-6 bg-rm-bg-elevated/40"
            : "h-auto py-2 justify-between bg-transparent",
        )}
      >
        <div
          className={cn(
            "hidden md:flex flex-1 items-center gap-2",
            hideExtraControls && "invisible",
          )}
        >
          {isCall && !isFullscreen && (
            <div
            ...
            >
              <button
                ...
                className={cn(
                  "w-10 h-10 md:w-12 md:h-10 rounded-xl flex items-center justify-center transition-all outline-none",
                  btnBaseClass,
                )}
              ...
              >
                <MessageSquare
                  size={20}
                  className={cn(!isChatHidden && "fill-rm-text/20")}
                />
              </button>
            </div>
          )}
        ...
        <div
          className={cn("flex items-center gap-2 md:gap-3 shrink-0 mx-auto")}
        >
          <div
            className={cn(
              "flex items-center gap-0.5 md:gap-1 p-1 rounded-2xl border shrink-0",
              pillBgClass,
            )}
          >
            <button
            ...
            >
              <Headphones
                size={20}
                className={isDeafened ? "text-destructive-foreground" : ""}
              />
            </button>
            ...
            >
              <div className="relative w-5 h-5 flex items-center justify-center">
                ...
                <div
                  className={cn(
                    "absolute inset-0 transition-all duration-300 ease-in-out flex items-center justify-center",
                    !focusedItem?.isStreaming && !activeActivity
                      ? "opacity-100 scale-100 rotate-0"
                      : "opacity-0 scale-75 -rotate-45",
                  )}
                >
        ...
        <div
          className={cn(
            "flex-1 flex items-center justify-end gap-1 md:gap-3 shrink-0",
            hideExtraControls && "invisible",
          )}
        >
          <div
            className={cn(
              "flex items-center gap-0.5 md:gap-1 p-1 rounded-2xl border",
              pillBgClass,
            )}
          >
            <button
