...
import { EyeOff, Headphones, MicOff, Volume2 } from "lucide-react";
  ...
  ({
    ...
    useEffect(() => {
      ...
      const loadDominantColor = async () => {
        ...
        const color = await extractDominantColor(
          getAuthAssetUrl(focusedItem.avatar),
        );
    ...
    if (focusedId && focusedItem) {
      ...
      const isPreviewHidden =
        isFocusedScreen &&
        focusedItem.isLocal &&
        !!voiceActions?.isPreviewHidden;
      const focusedThumbnailPoster =
        isFocusedScreen && !isPreviewHidden
          ? streamThumbnails[focusedItem.userId]
          : null;
      const isStreaming =
        (isFocusedCamera || isFocusedScreen) &&
        !!focusedItem.stream &&
        !isPreviewHidden;
      ...
      const showFocusedVolume =
        !focusedItem.isLocal &&
        (isFocusedScreen || isFocusedCamera) &&
        hasLiveAudioTrack(focusedItem.stream);
      ...
      return (
        <div className="w-full h-full flex flex-col items-center justify-center bg-rm-bg-primary overflow-hidden relative group/stage cursor-pointer hover:ring-2 hover:ring-rm-text/20 transition-all">
          ...
          <button
            ...
            style={{
              backgroundColor:
                isStreaming || focusedThumbnailPoster
                  ? "black"
                  : dominantColor || "var(--rm-bg-primary)",
            }}
          ...
          {isStreaming ? (
            <VideoPlayer
              ...
              muted={
                ...
                (focusedItem.isLocal
                  ? true
                  : !!currentSettings.peerSettings[focusedItem.userId]?.muted)
              }
                  ...
                  <div className="w-full h-full shadow-[0_30px_100px_rgba(0,0,0,0.6)] rounded-3xl md:rounded-[2.5rem] flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-3xl transition-all duration-500">
                    {focusedItem.avatar ? (
                      ...
                      <span className="text-8xl md:text-9xl font-black text-white">
                        {focusedItem.name[0]?.toUpperCase()}
                      </span>
                    )}
                    {isLoadingStream && (
              ...
              <div className="rounded-xl border border-rm-border bg-rm-bg-primary/60 shadow-xl backdrop-blur-md">
                ...
                <Slider
                  ...
                  onChange={(value) => {
                    ...
                    if (isFocusedScreen)
                      setPeerStreamVolume(focusedUserId, value);
              ...
              {(focusedItem.type === "screen" ||
                ...
                <span className="bg-rm-bg-surface/60 px-1.5 rounded-[3px] text-[8px] font-black text-rm-text uppercase tracking-tighter border border-rm-border tabular-nums">
                  <QualityMonitor
                    ...
                    signaledQuality={
                      focusedItem.isLocal && focusedItem.type === "screen"
                        ? voiceActions?.currentScreenQuality
                        : null
                    }
      ...
      return (
        ...
        >
          {items.map((item) => (
            <div
              key={item.id}
              className="relative flex-1 min-w-0 max-w-[400px] h-full flex items-center justify-center"
            >
              <div className="w-full flex items-center justify-center max-h-full aspect-video">
                <ParticipantCard
                  ...
                  onClick={() =>
                    onFocus(item.id === focusedId ? null : item.id)
                  }
                />
              </div>
            </div>
