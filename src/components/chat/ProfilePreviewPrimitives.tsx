...
export function ProfileSurfaceBackdrop({
  ...
  return (
    <div className={cn("pointer-events-none absolute inset-0 z-[1] overflow-hidden", className)}>
      <div className="absolute inset-0 opacity-[0.22]" style={{ background: "var(--rm-profile-custom-banner-fallback)" }} />
      {bannerUrl ? (
        <div className="absolute inset-0 opacity-[0.4]">
          <ProfileAssetLayer
            ...
            className={cn("object-contain object-center scale-[1.02]", assetClassName)}
          />
        </div>
      ) : null}
      <div className="absolute inset-0 opacity-80" style={{ background: "var(--rm-profile-custom-banner-overlay)" }} />
