  ...
  if (isBundleCollectible(item)) {
    ...
    return (
      <div className="relative flex h-full items-center justify-center overflow-visible bg-rm-bg-primary p-2 select-none">
        ...
        {effectItem && (
          <div className="absolute top-2.5 -right-1 w-[62px] h-[100px] overflow-hidden rounded-[6px] border border-white/8 bg-[#10131a] shadow-[0_8px_24px_rgba(0,0,0,0.5)] origin-top-right rotate-[4deg]">
            {/* Banner */}
        ...
        >
          ...
          >
            <HomeIcon
              className={
                effectItem ? "h-4 w-4 text-white/50" : "h-6 w-6 text-white/50"
              }
            />
            {decorationUrl && (
              <img
        ...
        {nameplateUrl && (
          <div
            className={cn(
              ...
              effectItem
                ? "left-2.5 bottom-3.5 w-[70px] h-[20px]"
                : "bottom-3 w-[90px] h-[24px] gap-1.5",
            )}
          >
            <img
              src={nameplateUrl}
            ...
            >
              <HomeIcon
                className={
                  effectItem
                    ? "h-2 w-2 text-white/50"
                    : "h-2.5 w-2.5 text-white/50"
                }
              />
  ...
  return (
    <article
      className={cn(
        ...
        isSelected
          ? "border-primary/70"
          : "border-rm-border hover:border-rm-border-strong",
      )}
    ...
    >
      ...
      <div className="space-y-3 p-3">
        <div className="min-h-[48px]">
          <div className="line-clamp-1 text-sm font-bold text-rm-text">
            {item.name}
          </div>
          <div className="line-clamp-1 text-xs text-rm-text-muted">
            {item.categoryName}
          </div>
          {price && (
            <div className="mt-1 text-[11px] font-semibold text-rm-text-secondary">
              {price}
            </div>
          )}
        </div>
...
export function CollectiblesCatalogModal({
  ...
  const [activeKind, setActiveKind] = useState<CollectibleKind>(
    initialKind ?? "avatar_decoration",
  );
  ...
  useEffect(() => {
    ...
    loadCollectiblesCatalog()
      ...
      .catch((err) => {
        if (!cancelled)
          setError(
            err instanceof Error ? err.message : "Unable to load collectibles.",
          );
      })
      .finally(() => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return (catalog?.items ?? [])
      ...
      .filter((item) => {
        ...
        return `${item.name} ${item.categoryName} ${item.label ?? ""}`
          .toLowerCase()
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unable to refresh collectibles.",
      );
    } finally {
      setRefreshing(false);
    }
  ...
  const handleApply = async (item: CollectibleCatalogItem | null) => {
    ...
    setError(null);
    try {
      const bundleSelections = item
        ? getCollectibleApplySelections(catalog, item)
        : [];
      ...
      <section className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-rm-border bg-rm-bg-primary shadow-2xl">
        <header className="flex items-center gap-3 border-b border-rm-border px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-base font-bold text-rm-text">
              Collectibles Catalog
            </h2>
            <p className="truncate text-xs text-rm-text-muted">
              {catalog
                ? `${catalog.items.length.toLocaleString()} items synced from ${catalog.source}`
                : "Loading catalog"}
            </p>
          </div>
          <Button
        ...
        <div className="border-b border-rm-border px-4 py-3">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
                ...
                >
                  ...
                  {catalog && (
                    <span className="ml-2 opacity-70">
                      {catalog.counts[kind].toLocaleString()}
                    </span>
                  )}
                </button>
              ))}
            ...
            <div className="relative flex-1">
              <Search
                size={15}
                className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-rm-text-muted"
              />
              <Input
                value={query}
