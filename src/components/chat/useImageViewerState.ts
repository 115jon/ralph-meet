...
function viewReducer(state: ViewState, action: ViewAction): ViewState {
  switch (action.type) {
    case "ZOOM_IN":
      return { ...state, scale: Math.min(state.scale + 0.5, 4) };
    case "ZOOM_OUT":
      return { ...state, scale: Math.max(state.scale - 0.5, 0.5) };
    case "RESET":
    ...
    case "TOGGLE_UI":
      return { ...state, hideUi: !state.hideUi };
    case "SET_HIDE_UI":
      return { ...state, hideUi: action.payload };
    case "SET_SCALE":
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (scale > 1) {
        ...
        viewDispatch({
          type: "START_DRAG",
          payload: { x: e.clientX - pan.x, y: e.clientY - pan.y },
        });
      }
    },
    [scale, pan],
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      if (isDragging && scale > 1) {
        ...
        viewDispatch({
          type: "SET_PAN",
          payload: getClampedPan(rawX, rawY, scale),
        });
      }
    },
