    ...
    (file: File) => {
      ...
      if (!file.type.startsWith("image/")) {
        dispatch({
          ...
          payload: "Only image files are allowed",
        });
        return;
      }
      // Validate size (8MB)
      if (file.size > 8 * 1024 * 1024) {
        dispatch({
          type: "SET_UPLOAD_ERROR",
          payload: "Image too large (max 8MB)",
        });
        return;
      }
    ...
    try {
      ...
      if (state.iconFile) {
        ...
        try {
          ...
          dispatch({
            type: "SET_UPLOAD_ERROR",
            payload: (err as Error).message || "Failed to upload icon",
          });
  ...
  return (
    <BaseModal onClose={onClose}>
      ...
      >
        ...
        <div
          className={cn(
            "relative z-10 w-full sm:max-w-md h-[90vh] sm:h-auto animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-0 sm:fade-in sm:zoom-in-95 rounded-t-[24px] sm:rounded-2xl border border-rm-border bg-rm-bg-primary p-6 shadow-2xl duration-300 sm:duration-200 pointer-events-auto flex flex-col",
            isClosing &&
              "animate-out slide-out-to-bottom-full sm:slide-out-to-bottom-0 sm:fade-out sm:zoom-out-95",
          )}
        >
          {/* Mobile drag handle */}
          ...
          <h2 className="mb-1 text-center text-xl font-bold text-rm-text">
            Create a Server
          </h2>
          ...
          {state.uploadError && (
            <p className="mb-3 text-center text-xs font-medium text-red-800 dark:text-red-400">
              {state.uploadError}
            </p>
          )}
