...
function renderInline(
  ...
  const regex =
    /(`[^`]+`|\*\*[^*]+\*\*|\*[^*]+\*|~~[^~]+~~|\[[^\]]+\]\([^)]+\)|<:[a-z0-9_]+:[a-z0-9-]+>|:[a-z0-9_+-]+:|@([a-zA-Z0-9_]+)|(https?:\/\/[^\s<>"'`)\]]+))/gi;
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (m.startsWith("`")) {
      parts.push(
        <code
          key={`ic-${match.index}`}
          className="rounded bg-rm-bg-elevated px-1.5 py-0.5 text-[0.85em] text-primary"
        >
          {m.slice(1, -1)}
        </code>,
      );
      ...
      if (isInsideUrl(text, match.index)) {
        ...
        parts.push(
          nativeEmoji ? (
            <EmojiToken key={`ne-${match.index}`} value={m} selectable />
          ) : (
            m
          ),
        );
...
export const MarkdownRenderer = React.memo(({ content }: Props) => {
  ...
  while ((match = codeBlockRegex.exec(content)) !== null) {
    if (match.index > lastIndex) {
      parts.push(
        ...renderInline(content.slice(lastIndex, match.index), customEmojiMap),
      );
    }
    parts.push(
      <pre
        key={`cb-${match.index}`}
        className="my-1.5 overflow-x-auto rounded-lg bg-rm-bg-elevated p-3 text-xs leading-relaxed text-rm-text-secondary border border-rm-border"
      >
