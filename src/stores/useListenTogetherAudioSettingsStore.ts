...
export function normalizeListenTogetherAudioSettings(
  ...
  return {
    ...
    preset: settings?.preset && settings.preset in LISTEN_TOGETHER_LOUDNESS_PRESETS
      ? settings.preset
      : DEFAULT_LISTEN_TOGETHER_AUDIO_SETTINGS.preset,
  };
}

export const useListenTogetherAudioSettingsStore = create<ListenTogetherAudioSettingsState>()(
  persist(
    (set) => ({
      ...DEFAULT_LISTEN_TOGETHER_AUDIO_SETTINGS,
      updateSettings: (updates) => set((state) => normalizeListenTogetherAudioSettings({
    ...
    }),
    {
      ...
      version: 1,
      partialize: (state) => ({ enabled: state.enabled, preset: state.preset }),
