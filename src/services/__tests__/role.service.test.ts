...
describe("createRole", () => {
  ...
  beforeEach(() => {
    ...
    db.mockQuery(
      ...
      roleRow({ id: "new-role-id" }),
    );
    db.mockQuery(
      /SELECT r\.permissions, r\.position, s\.owner_id/,
      {
        results: [
          { permissions: MANAGE_ROLES, position: 10, owner_id: "someone_else" },
        ],
      },
      [SERVER_ID, USER_ID],
    );
  ...
  it("throws 403 when non-owner tries to create an administrator role", async () => {
    await expect(
      createRole(db as any, SERVER_ID, USER_ID, {
        name: "Admin",
...
describe("updateRole", () => {
  ...
  it("updates a role when actor has MANAGE_ROLES", async () => {
    ...
    const result = await updateRole(db as any, SERVER_ID, ROLE_ID, USER_ID, {
      name: "Updated",
    });
  ...
  it("throws 403 when non-owner tries to elevate a role to administrator", async () => {
    ...
    await expect(
      updateRole(db as any, SERVER_ID, ROLE_ID, USER_ID, {
        permissions: ADMINISTRATOR,
      }),
    ).rejects.toHaveProperty("status", 403);
  });
...
describe("updateMemberRoles", () => {
  ...
  it("throws 403 when non-owner tries to assign an administrator role", async () => {
    ...
    await expect(
      updateMemberRoles(db as any, SERVER_ID, TARGET_USER_ID, USER_ID, [
        ROLE_ID,
      ]),
    ).rejects.toHaveProperty("status", 403);
  });
