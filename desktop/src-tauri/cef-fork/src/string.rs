...
impl From<&CefStringUserfreeUtf16> for CefStringUtf16 {
    fn from(value: &CefStringUserfreeUtf16) -> Self {
        ...
        // NOTE (ralph-meet vendored patch): an empty/null userfree string
        // (`value.is_none()`) is a perfectly normal, expected value — CEF
        // returns it for countless empty fields on every navigation/IPC. The
        // upstream crate eprintln!'d "Invalid UTF-16 string" here, which spammed
        // stderr hundreds of times on startup. Only a genuine
        // `cef_string_utf16_set` FAILURE on a non-empty source is an actual
        // error, so the diagnostic is moved into that branch (matches the
        // upstream cef-rs fix PR).
        Self(CefStringData::Clear(value.and_then(|value| unsafe {
            let mut data = mem::zeroed();
