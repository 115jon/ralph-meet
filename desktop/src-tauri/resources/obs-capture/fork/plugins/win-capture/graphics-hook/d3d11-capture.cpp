...
static bool create_d3d11_tex(uint32_t cx, uint32_t cy, ID3D11Texture2D **tex, HANDLE *handle)
{
	...
	// SHADER_RESOURCE is what stock OBS needs to sample the shared texture.
	// RENDER_TARGET is a fork addition: the host's encoder feeds this shared
	// surface straight into a D3D11 VideoProcessor, whose input view REQUIRES
	// BIND_RENDER_TARGET. Without it the host must do a full-frame same-device
	// CopyResource into an owned RT texture every frame (the "[VP] normalizing
	// hook surface" path) — pure per-frame GPU overhead on the device shared
	// with the game. Adding the bind here lets the host bind the shared surface
	// directly (zero extra copy). Both flags are legal together on a shared tex.
...
static inline void d3d11_shtex_capture(ID3D11Resource *backbuffer)
{
	if (data.texture) {
		 ...
		 * present-accurate frame signal so the host forwards exactly this
		 * frame (no duplicate re-encodes). */
